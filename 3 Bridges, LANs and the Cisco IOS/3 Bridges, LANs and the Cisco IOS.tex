...
\section{Network setup}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{ *7l }
            ...
            \hline 
            1 & 128.238.61.1/24 & 128.238.61.2/24 & h0(shakti) & 128.238.66.100/24 & h1 (vayu) & 128.238.66.101/24 \\
            2 & 128.238.62.2/24 & 128.238.62.3/24 & h2(agni) & 128.238.66.102/24 & h3 (apah) & 128.238.66.103/24 \\
            3 & 128.238.63.3/24 & 128.238.63.4/24 & h4(yachi) & 128.238.66.104/24 & h5 (fenchi) & 128.238.66.105/24 \\
            4 & 128.238.64.4/24 & 128.238.64.5/24 & h6(kenchi) & 128.238.66.106/24 & h7 (guchi) & 128.238.66.107/24 \\
            \hline \hline
            \end{tabular}
    \end{table}
...
\section{IOS Console}
    Open Router Console (Use telnet tow router in physical devices).
...
\section{Bridge packet}
    ...
    \begin{enumerate}
        \item \texttt{config term}
        \item \texttt{no ip routing}
        \item \texttt{bridge 1 protocol ieee} \textit{for STP protocol}
        \item \texttt{int f0/0}
        \item \texttt{ip addr 128.238.61.1 255.255.255.0}
        \item \texttt{bridge-group 1}
        \item \texttt{no shut}
        \item \texttt{int f0/1}
        \item \texttt{ip addr 128.238.61.2 255.255.255.0}
        \item \texttt{bridge-group 1}
        \item \texttt{no shut}
        \item \texttt{Ctrl+Z}
    \end{enumerate}
    ...
    You should collect eight different BPDUs in this exercise for all group.
    ...
    Identify the values of root ID, root path cost, bridge ID, and port ID for each BPDU\footnote{You may check the physical addresses of network interfaces. You need the MAC addresses to help analyze the BPDUs.}.
...
\section{Cisco IOS HTTP rest API}
    ...
    \begin{verbatim}
        ...
        R1(config)#username test secret test
