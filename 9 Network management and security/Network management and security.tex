...
\section{Plain Transfer}
    ...
    Execute \lstinline[emph={h0,netlab}]{ftp h0.netlab} on the \textit{h1}.
...
\section{Secure Transfer}
    Start \lstinline{ssh} service on the \textit{h0} by \lstinline{service ssh start}.
    
    Execute \lstinline{tcpdump -enX -s 100} or \lstinline{wireshark} on the \textit{h1} host or the \textit{h0} host to capture packets between two machine.

    Execute \lstinline[emph={1111,h0,1111@h0,netlab},escapechar=]{sftp 1111@h0.netlab} on the \textit{h1}.
