...
\section{ARP Table}
    ...
    If you find that all the remote hosts are in your host’s ARP table, you need to delete a remote host from the table, using:
    \footnote{If you deleted your host’s IP address from the ARP table by mistake, you must add the entry back in the table. See the \lstinline{arp} manual page to add.
    Note that, in order for your host to reply to the ARP requests, the ARP entry of your host must have the \lstinline{P} flag in the ARP table.}
    \begin{lstlisting}[emph={remote-host}]
arp -d remote-host
    \end{lstlisting}
...
\section{ARP Proxy}
    The network topology for this proxy ARP exercise is shown in \hyperref[fig:2.9]{Figure~2.9}. The IP addresses and network masks for the hosts are also given in \hyperref[fig:2.9]{Figure~2.9}. Change the IP address and network mask of your host accordingly (see Section~2.3.2 of reference book). Change the IP addresses and network masks of the \textit{Router4} interfaces according to \hyperref[fig:2.9]{Figure~2.9}.
...
\section{Gratuitous (Unsolicited) ARP}
    While \lstinline{tcpdump -ex} (or run \lstinline{wireshark}) is running on all the hosts, reboot host \textit{h7}. You can send the gratuitous ARP manually by execute:
...
\section{\texttt{ping} ICMP}
    ...
    \item List the different ICMP messages you captured in \nameref{sec:icmp-ping}. Give the values of the type and code fields.
    \end{report}
