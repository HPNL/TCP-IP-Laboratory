...
socket -i -u -n1 -w1000 host-in-64-0-subnet echo
    ...
    \begin{figure}[H]
        \centering
            ...
            \end{tikzpicture}
        \caption{Network configuration (Figure~2.9)}
    \end{figure}
...
\part{Exercises with IP address and subnet mask}
    ...
    \begin{table}[H]
        \caption{Host IP addresses and network masks for \textit{11 Duplicate IP (Table~2.3.)}}
        \centering
...
\section{IP Subnets}
    ...
    \begin{table}[H]
        \caption{Host IP addresses and network masks for \textit{12 IP subnet (Table~2.4.)}}
        \centering
        \begin{tabular}{ c c c }
            \hline \hline
