...
\section{Local Host Dump}
    ...
    Can you see any ICMP message sent from \textit{h0} in the \lstinline{tcpdump} output?
    Why?
    \end{report}
...
\section{ARP Table}
    ...
    \footnote{If you deleted \textit{h0}’s IP address from the ARP table by mistake, you must add the entry back in the table. See the \lstinline{arp} manual page to add.
...
\section{ARP Proxy}
    ...
    \begin{enumerate}
        \item \lstinline{telnet} to \textit{Router4}. (Right click on the Router and open console on GNS3 or use \lstinline{telnet 128.238.64.4}.)
...
socket -i -u -n1 -w1000 host-in-64-0-subnet echo
    ...
    Run \lstinline{arp -a} to display the new ARP table in \textit{h1}, \textit{h4} and \textit{h5}.
    Save the ARP table for your lab report.
