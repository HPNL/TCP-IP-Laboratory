...
\boxbashaux{{h\textsubscript{0}}'s Auxiliary Console }
\begin{lstlisting}[emph=netlab]
telnet 128.238.66.101 # using user: netlab and password: netlab
# after login to h1
netstat -ie
\end{lstlisting}
...
\section{ARP Timeout}
While \lstbash[emph={h0,netlab}]{tcpdump host h0.netlab} is running to capture traffic from the \textit{h0} machine, execute \lstbash{telnet 128.238.66.200}.
...
\begin{enumerate}
    \item \lstbash{telnet} to \textit{Router4}.
          (Right click on the Router and open console on GNS3 or use \lstbash{telnet 128.238.64.4}.)
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \draw (0*2,3) node[draw,fill=\colorhost] (h0) {h0} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 64.100/24};
        \draw (1*2,3) node[draw,fill=\colorhost] (h1) {h1} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 64.101/24};
        \draw (2*2,3) node[draw,fill=\colorhost] (h2) {h2} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 64.102/24};
        \draw (3*2,3) node[draw,fill=\colorhost] (h3) {h3} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 64.103/24};
        \draw (0*2,0) node[draw,fill=\colorhost] (h4) {h4} -- +(0,+0.75) +(0,-0.5) node {\scriptsize 65.104/16};
        \draw (1*2,0) node[draw,fill=\colorhost] (h5) {h5} -- +(0,+0.75) +(0,-0.5) node {\scriptsize 65.105/16};
        \draw (2*2,0) node[draw,fill=\colorhost] (h6) {h6} -- +(0,+0.75) +(0,-0.5) node {\scriptsize 65.106/16};
        \draw (3*2,0) node[draw,fill=\colorhost] (h7) {h7} -- +(0,+0.75) +(0,-0.5) node {\scriptsize 65.107/16};
        \draw (0.5*2,1.5) node[draw,rounded corners,fill=\colorcisco] (r4) {Router 4}
        -- +(0,+0.75) +(0,+0.45) node {\scriptsize\textit{eth0}\quad 64.4/24}
        -- +(0,-0.75) +(0,-0.45) node {\scriptsize\textit{eth1}\quad 65.4/24}
        ;
...
\section{ping ICMP}
Use \lstbash[emph={h1,netlab}]{ping -sv h1.netlab} on the \textit{h0} to test whether the \textit{h1} host is reachable, while running:\\
\lstbash[emph={h0,h1,netlab},morekeywords={[3]host,and}]{tcpdump -enx host h0.netlab and h1.netlab}.
...
While running \lstbash[emph={h0,h1,netlab}]{tcpdump -x -s 70 host h0.netlab and h1.netlab}, execute the following \lstbash{socket} command to send a UDP datagram to the \textit{h1} host from the \textit{h0}:
