...
socket -i -u -n1 -w1000 host-in-64-0-subnet echo
    ...
    \begin{figure}[H]
        \centering
            \begin{tikzpicture}
                \draw (0*2,3) node[draw,fill=white] (h0) {h0} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 65.100/16};
                \draw (1*2,3) node[draw,fill=white] (h1) {h1} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 65.101/16};
                \draw (2*2,3) node[draw,fill=white] (h2) {h2} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 65.102/16};
                \draw (3*2,3) node[draw,fill=white] (h3) {h3} -- +(0,-0.75) +(0,+0.5) node {\scriptsize 65.103/16};
            ...
            \end{tikzpicture}
        \caption{\textit{Figure 2.9} Network configuration}
    \end{figure}
...
\section{Duplicate IP}
    ...
    \begin{enumerate}
        \item Execute \lstinline{telnet} from one of two hosts with the duplicate IP address to a host with unique IP address (e.g.\ \textit{h0} $\rightarrow$ \textit{h2} in Group A and \textit{h4} $\rightarrow$ \textit{h6} in Group B).

        Now, from the other host with the duplicate IP address, execute \lstinline{telnet} command to the same host (\textit{h1} $\rightarrow$ \textit{h2} or \textit{h5} $\rightarrow$ \textit{h6}).
...
\section{IP Subnets}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{ c c c c }
            \hline \hline
            Group & Host & IP Address & \makebox[7.3em][c]{Subnet Mask} \\
            \hline 
            Group A & h0 & 128.238.66.100 & \makebox[7.3em][l]{255.255.255.240} \\
                    ...
                    \hline
            Group B & h4 & 128.238.66.104 & \makebox[7.3em][l]{255.255.255.240} \\
                    & h5 & 128.238.66.105 & \makebox[7.3em][l]{255.255.255.0} \\
                    & h6 & 128.238.66.106 & \makebox[7.3em][l]{255.255.255.0} \\
                    & h7 & 128.238.66.121 & \makebox[7.3em][l]{255.255.255.240} \\
            \hline \hline
            \end{tabular}
    \end{table}
