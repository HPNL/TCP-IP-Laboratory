...
more /etc/services
    ...
    Display the file sizes using \lstinline{ls -l ser*}. Save the output. 
...
\section{Network Command Manual}
    ...
    \begin{report}
    \item Explain the above commands briefly. (Two or three sentences per command would be adequate.)
    \end{report}
...
\section{\texttt{ARPing}}
    This time we will run \lstinline{wireshark} to capture an ARP request and an ARP reply in real-time. Simply run \lstinline{wireshark &} in \textit{h1} and select the interface and start capturing.
