...
\section{Multicast \texttt{ping}}
    Execute \lstinline{ping 224.0.0.1} on the \textit{h1}.
...
\section{Multicast vs Unicast}
    on the \textit{h1} execute \lstinline{tcpdump -n -nn -e} and \lstinline[morekeywords={[3]ether,multicast}]{tcpdump ether multicast -n -nn -e} (or run \lstinline{wireshark}) to capture an Ethernet unicast frame, an Ethernet multicast frame, and an Ethernet broadcast frame.

    To generate an Ethernet unicast frame, run \lstinline[emph={h1,netlab}]{socket -i -u -n1 h1.netlab echo} from the \textit{h0}.
...
./netspy 224.111.111.111 1500
    \end{lstlisting}
    Then, start the multicast sender \lstinline{netspyd} on the \textit{h0}, by executing:
    \begin{lstlisting}
./netspyd 224.111.111.111 1500 1
    \end{lstlisting}
    Execute \lstinline{tcpdump ip multicast} or \lstinline{wireshark} on every subnet to capture multicast IP datagrams.
...
\section{\texttt{ping} Replay}
    ...
    Execute \lstinline{ping 224.111.111.111} from the \textit{h6}.
    ...
    Terminate the \lstinline{netspy} programs on several hosts, e.g.\ \textit{h0}, \textit{h1}, and the \textit{h5}.
...
./netspy 224.111.111.111 1500
    \end{lstlisting}
    Start \lstinline{netspy} on the \textit{h1}, by using:
    \begin{lstlisting}
...
\section{IGMP Types}
    Execute \lstinline[morekeywords={[3]multicast}]{tcpdump ip multicast -v} or \lstinline{wireshark} in one console to capture IGMP messages.
...
% FIXME: check senario
    ...
    Terminate \lstinline{netspy} on the \textit{h2} and the \textit{h4}.
...
\section{Multicast Multi-Hub}
    ...
    Run \lstinline[morekeywords={[3]multicast}]{tcpdump ip multicast} or \lstinline{wireshark} on all subnet (at least one host).
    Maybe you need to restart (or reload) routers to capture all packet from start state.

    Execute \lstinline{./netspy 224.111.111.111 1500} on the \textit{h0}, \textit{h2}, \textit{h3}, \textit{h5}, and the \textit{h6}.

    Execute \lstinline{./netspyd 224.111.111.111 1500 16} on the \textit{h4}.
    
    To generate multicast traffic, you can login (by \lstinline{telnet} or \lstinline{ssh}\footnote{Need to \lstinline{service ssh start} on remote machine}) to or logout of \textit{h4}.
    Each time when the login user set of \textit{h4} changes, \lstinline{netspyd} on the \textit{h4} will send a multicast datagram to group 224.111.111.111, to report the change in its login users.

    Can you see the \lstinline{netspy} messages on the 128.238.65.0 subnet in the \lstinline{tcpdump} output? 

    Terminate the \lstinline{netspy} program on the \textit{h6}.
...
\section{Multicast Tree}
    ...
    \begin{report}
        \item Report the \lstinline[language={cisco}]{mstat}\footnote{\lstinline[language={cisco},emph={x}]{mstat x.x.x.x}} output for the \textit{h4} on \textit{R1}.
    \end{report}
...
\section{**Multicast Real-Time Video}
    ...
    on the \textit{h0}, go to the \lstinline{vlc} menu: \textit{Media/Stream\ldots }.
    ...
    Now the \lstinline{vlc} on the \textit{h0} is transmitting the video clip using \textit{RTP/RTSP/UDP/IP} to the multicast group {224.123.111.101} on port {22224}.
    ...
    Execute \lstinline[morekeywords={[3]multicast}]{tcpdump ip multicast} or \lstinline{wireshark} in one console to capture the multicast datagrams.
    In another console, execute \lstinline[morekeywords={[3]multicast}]{tcpdump ip multicast} to monitor the capture process.
