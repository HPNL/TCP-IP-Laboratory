...
\section{Simple UDP Multicast Client and Server}
    Go to \textit{code} directory for \textit{netlab} user at \path{/home/netlab/code} with \lstbash{cd} command.
...
./netspy 224.111.111.111 1500
    \end{lstlisting}
    Then, start the multicast sender \lstbash{netspyd} on the \textit{h0}, by executing:
    \begin{lstlisting}
...
\label{sec:config-router}
    ...
    Login to the router and run \lstbash{ip multicast-routing} to enable multicast routing in the \textit{Global Configuration} mode.
    Then, enable the PIM protocol on each interface, by running \lstbash{ip pim dense-mode} in the \textit{Interface Configuration} mode.
    Now the router is enabled to do multicast routing using PIM.

    Login to the router, execute \lstbash{show ip igmp interface} and \lstbash{show ip igmp group} in the \textit{Privileged EXEC} mode.
...
\section{IGMP Types}
    ...
    When you see three or more IGMP queries in the second \lstbash{tcpdump} output, terminate both \lstbash{tcpdump} programs.
% FIXME: check senario
