...
\part{A Simple Router Experiment}
    As in the previous lab, we will divide the students into four groups, each with two workstations, a router, and two hubs, which are to be connected as shown in \hyperref[fig:4.10]{Figure~4.10}.
    The IP addresses of the routers and hosts are given in \hyperref[tab:4.2]{Table~4.2} and \hyperref[tab:4.3]{Table~4.3}, respectively.
    \begin{table}[H]
...
\section{Simple Routing}
    Configure the IP addresses of your workstations and the router as shown in \hyperref[fig:4.10]{Figure~4.10}, \hyperref[tab:4.2]{Table~4.2} and \hyperref[tab:4.3]{Table~4.3} (select one of row in table).
...
Router(config)# no router rip
    ...
    Configure network router and workstation’s as \hyperref[fig:4.11]{Figure~4.11}. Since our workstation’s don’t have \lstinline{routed} package, you need to add route for subnet \textit{128.238.0.0/16} to each workstation.
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
...
\section{RIP Messages}
    Connect the routers and hosts and change the IP addresses of the workstations and router4 as shown in \hyperref[fig:4.11]{Figure~4.11}.
...
\section{**ICMP router discovery}\label{sec:ICMPRD}
    ...
    Telnet to the routers, change the IP address of the \textit{ethernet1} interfaces as shown in \hyperref[fig:4.12]{Figure~4.12}. Enable ICMP router discovery on these two interfaces by the following
...
\section{OSPF Messages}
    After connecting the cables properly, change the host IP addresses as given in \hyperref[fig:4.13]{Figure~4.13} You need to remove the default route added in Exercise 4 from the host routing table.
...
\section{Static routing}
    ...
    Check the IP addresses of the workstations as shown in \hyperref[fig:4.13]{Figure~4.13}. 
...
\section{\texttt{traceroute} multi-hop path}
    Execute \lstinline[emph={your-host, remote-host}]{tcpdump -env host your-host and remote-host} on your host, where \textit{remote-host} is a workstation at least two hops away. 
