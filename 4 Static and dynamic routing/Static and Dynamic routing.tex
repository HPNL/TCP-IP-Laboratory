...
\section{Router Discovery}
    ...
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
            \draw (0,0) node[draw,align=center,rounded corners] (s) {Core\\Switch};
            ...
            \draw (-3.25,2) node[draw,align=center,rounded corners] (r3) {Router 3} +(0,-0.5) node {\scriptsize\textit{ethernet1} 66.3/24~~~};
            \draw (-3.25,-2) node[draw,align=center,rounded corners] (r4) {Router 4} +(0,0.5) node {\scriptsize\textit{ethernet1} 66.4/24~~~};
            \draw (-2.5,0) node[draw] (h0) {h0} +(0,-0.5) node {\scriptsize 66.100/24};
            
            \draw[dotted] (s) -- +(2.5,1) node[anchor=south west] {\scriptsize 128.238.66.0/24 subnet};
            \draw (r3) -- +(0,-0.75) -- (s);
            \draw (r4) -- +(0,0.75) -- (s);
            \draw (s) -- (h0);
            \draw (h1) -- (s);
            ...
            \draw (h7) -- (s);
        \end{tikzpicture}
        \caption{\textit{Figure 4.12} Network configuration of the ICMP router discovery experiment}        
    \end{figure}
    
    Telnet to the routers, change the IP address of the \textit{ethernet1} interfaces as shown in Fig. 4.12. Enable ICMP router discovery on these two interfaces by the following
...
Router(config-router)# network address wildcard-mask area area-id
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            \draw[thick] (-3.5,-2) -- +(7,0) node[pos=0.5,above] {\scriptsize 128.238.63.0 subnet};
        \end{tikzpicture}
        \caption{\textit{Figure 4.13} Network Configuration for OSPF exercises, static routing experiments and \lstinline{traceroute} experiment}
    \end{figure}
