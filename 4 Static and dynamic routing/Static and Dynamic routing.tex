...
tcpdump -en host h1.netlab and h0.netlab
    \end{lstlisting}
    Send \lstinline{ping} messages continuously from \textit{h0} to \textit{h1}:
...
ping -sv h1.netlab
    ...
    \begin{report}
        ...
        \item What are the source and destination addresses in the IP and Ethernet headers of a packet that went from \textit{h0} to the router?
        ...
        \item Answer the above two questions, but now for the echo reply that was returned from \textit{h1} machine.
...
tcpdump -x -s 120 ip proto 89
    ...
    \begin{report}
        ...
        \item Can you ping \textit{h1} from \textit{h2}?
...
\section{\texttt{traceroute} multi-hop path}
    Execute \lstinline[emph={your-host, remote-host},morekeywords={[3]host,and}]{tcpdump -env host your-host and remote-host} on your host, where \textit{remote-host} is a host at least two hops away (for example \textit{h1} to \textit{h2} in \hyperref[fig:4.13]{Figure~4.13}).
