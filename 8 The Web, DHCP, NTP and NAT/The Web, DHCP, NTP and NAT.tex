...
\section{** NTP Server}
    Keep the NTP server running on \textit{h0}.
    Execute \lstinline[emph={your-host, remote-host}]{tcpdump -exn -nn host your-host and 128.238.66.100} or \lstinline{wireshark} to capture the NTP messages between \textit{your-host} and \textit{h0}.
...
\section{NAT}
    ...
    Note that there is a static translation that maps 10.0.0.7, or \textit{h7}, to 128.238.61.104.
...
\section{FTP Socket Programming}
    ...
    \begin{itemize}
        ...
        Also, you can try the commands out in the \lstinline{telnet} window, e.g.\ use \lstinline[emph={netlab}]{USER netlab} to send the user ID and \lstinline[emph={netlab}]{PASS netlab} to send the password to the FTP server.
