...
\section{HTTP Request}
    Execute \lstinline{wireshark} to capture packets between the \textit{gui} host and the \textit{server} host.

    Login to the \textit{server} host’s web server: \lstinline[emph={server,netlab}]{telnet server.netlab 80}.
...
\section{HTTP \texttt{Keep-Alive}}
    ...
    Execute \lstinline[emph={gui,server,netlab},morekeywords={[3],host,and}]{tcpdump host gui.netlab and server.netlab} or run \lstinline{wireshark} to capture packets between the \textit{gui} host and the \textit{server} host.
    ...
    Use \lstinline[emph={gui,server,netlab},morekeywords={[3],host,and}]{tcpdump host gui.netlab and server.netlab} or run \lstinline{wireshark} and print the HTTP requests and responses for the lab report.
...
\section{DHCP Server}
    ...
    \begin{enumerate}
        ...
        \item Start the DHCP server on the \textit{h1} in the foreground and working in the debugging mode: \lstinline{dhcpd -d -f}\footnote{default }.
...
\section{Remote Date}
    While \lstinline[emph={h0,h1,netlab}]{tcpdump -n -nn -ex host h0.netlab and h1.netlab} or \lstinline{wireshark} is running, execute \lstinline[emph={h1,netlab}]{rdate -p h1.netlab} on the \textit{h0} to display the system time of the \textit{h1} machine.
...
\section{NTP Sync}
    ...
    Start the NTP server on the \textit{h0} by \lstinline{service ntp start}.
    To determine the status of the NTP server, use \lstinline{service ntp status}.
    Use \lstinline[emph={h0,h1,netlab},morekeywords={[3],host,and}]{tcpdump -ex -n -nn host h1.netlab and h0.netlab} or \lstinline{wireshark} to capture packets between the \textit{h1} host and the \textit{h0}.
    Execute \lstinline{ntpdate -d -v 128.238.66.100} to synchronize the \textit{h1} host with \textit{h0}.
...
\section{** NTP Server}
    ...
    Execute \lstinline[emph={h1,h0,netlab},morekeywords={[3]host,and}]{tcpdump -exn -nn host h1.netlab and h0.netlab} or \lstinline{wireshark} to capture the NTP messages between the \textit{h1} host and the \textit{h0}.
...
\section{NAT}
    ...
    One student should \lstinline{telnet} to the router and configure the router as shown in \hyperref[tab:8.5]{Table~8.5}.
    Note that there is a static translation that maps 10.0.0.7, or the \textit{h7}, to 128.238.61.104.
    Login to the router, execute \lstinline[language={cisco}]{write term} to display the current router configuration.
    Execute \lstinline[language={cisco}]{show ip nat translations} in the \textit{Privileged EXEC} mode to display the translation table.
...
\section{NAT Visibility}
    ...
    Let an inside host send packets to an outside host, e.g.\ from the \textit{h5}, execute \lstinline{ping} 128.238.61.100.
...
\section{NAT Table}
    ...
    Execute \lstinline{socket -i -u -n1 128.238.61.101 8888} on the \textit{h2} to generate an ICMP port unreachable error.
...
\section{Socket Programming}
    ...
    Start \lstinline[emph={h0,h1,netlab,remote-host}]{tcpdump host remote-host} or \lstinline{wireshark} to capture packets from or to a remote host.

    On the remote host, start the UDP server by \lstinline[emph={server-port}]{./UDPserver server-port}.
    Then, start the UDP client on your host by \lstinline[emph={h0,h1,netlab,remote-host, server-port, a-message}]{./UDPclient remote-host server-port a-message}.
