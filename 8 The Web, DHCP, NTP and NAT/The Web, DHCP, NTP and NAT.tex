...
\section{DHCP server}
    ...
    \begin{enumerate}
        ...
        When \textit{shakti} is successfully reconfigured, execute \lstinline{ifconfig -a} or \lstinline{ip a} to display its
...
\section{NAT}
    ...
    One student should \lstinline{telnet} to the router and configure the router as shown in Table 8.5. Note that there is a static translation that maps 10.0.0.7, or \textit{guchi}, to 128.238.61.104.
...
\section{NAT Visibility}
    ...
    Before any host in the private network send any packets out, \lstinline{ping} an inside host (e.g. \textit{fenchi}) from an outside host (e.g. \textit{vayu}). You may try to \lstinline{ping} \textit{10.0.0.5, 128.238.61.102, 128.238.61.103, or 128.238.61.104}. Can you \lstinline{ping} these IP addresses? 

    Let an inside host send packets to an outside host, e.g. from \textit{fenchi}, execute \lstinline{ping} 128.238.61.100. Can you \lstinline{ping} \textit{fenchi} from an outside host now? Why? Which IP address should be used in the \lstinline{ping} command in order to \lstinline{ping} \textit{fenchi}? 
...
\section{NAT Table}
    ...
    Execute \lstinline{socket -i -u -n1 128.238.61.101 8888} on \textit{agni} to generate an ICMP port unreachable error. 
...
\section{NAT and PAT}
    ...
    Start the Apache web server on \textit{guchi}. Also, start the web browser \textit{Mozilla} on an outside host (e.g. \textit{shakti}), and enter the URL \url{http://128.238.61.1}. Save the \lstinline{tcpdump} output.
