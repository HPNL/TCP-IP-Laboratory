...
\section*{HTTP exercises}
    For the exercises in this section, the network topology is simple with two hosts are connected to a single network segment with IP addresses, i.e., from 128.238.66.100 to 128.238.66.101. Add first host from \texttt{http-server} and another from \texttt{gui}.
...
\section{HTTP Server}
    ...
    \begin{enumerate}
        ...
        Which one was the master server, and which ones were the child servers? (\texttt{use \textbf{htop} to see process hierarchy})
...
\section*{DHCP exercises}
    For the exercises in this section, the network topology is given in Fig. 1.3, where all the hosts are connected to a single network segment using their default IP addresses, i.e., from 128.238.66.100 to 128.238.66.107.
...
\section*{NTP exercises}
    Before proceeding to the next exercise, reboot the hosts and set network topology is given in Fig. 1.3.
...
\section{NTP Sync}
    In this exercise, we start the NTP server daemon on \texttt{h1} and use NTP to
    synchronize all the other hosts to \texttt{h1}.
    Study the NTP configuration file \texttt{/etc/ntp.conf} in \texttt{h1} and in your host.
    If you are using another machine, you can telnet to \texttt{h1} and display the \texttt{/etc/ntp.conf} file in the telnet window.
    Start the NTP server on \texttt{h1} by: \textbf{service ntp start}.
    To determine the status of the NTP server, use \textbf{service ntp status}.
    Use \textbf{tcpdump -ex -n -nn host \textit{your\_host} and \textit{h1}} or \textbf{wireshark} to capture packets between your host and \texttt{h1}.
    Execute \textbf{ntpdate -d -v 128.238.66.100} to synchronize your host to \texttt{h1}.
...
\section{NTP server}
    Keep the NTP server running on \texttt{h1}. Execute \textbf{tcpdump -exn -nn host \textit{your\_host} and \textit{128.238.66.100}} or \textbf{wireshark} to capture the NTP messages between \texttt{your\_host} and \texttt{h1}.\\
    Config \texttt{/etc/ntp.conf} in \texttt{your\_host} and add \textbf{server 128.238.66.103} to config file.\\
